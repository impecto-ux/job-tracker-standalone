...
import { X, Shield, Users, UserPlus, UserMinus, Edit2, Check, Layout, Activity, MessageSquare, Plus, Building, Search, AlertTriangle, Globe } from 'lucide-react';
        ...
        <AnimatePresence>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-2xl bg-[#222e35] rounded-2xl shadow-2xl border border-white/10 overflow-hidden flex flex-col max-h-[90vh] no-scrollbar"
                >
                    {/* Header */}
                    ...
                    {/* Navigation Tabs */}
                    <div className="flex items-center gap-1 border-b border-white/5 px-6 no-scrollbar overflow-x-auto">
                        <button
                                    ...
                                    <section className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-2">
                                            {[...(groupDetails?.users || [])].sort((a, b) => {
                                                const aAdmin = groupDetails?.adminIds?.includes(a.id) ? 1 : 0;
                                                const bAdmin = groupDetails?.adminIds?.includes(b.id) ? 1 : 0;
                                                ...
                                                <select
                                                    ...
                                                    onChange={async (e) => {
                                                        ...
                                                        try {
                                                            ...
                                                            // We don't strictly need to re-fetch if optimistic update is correct, 
                                            ...
                                            {isEditing ? (
                                                ...
                                                <>
                                                    <div className="flex items-start justify-between">
                                                        <div>
                                                            <div className="text-2xl font-bold text-white mb-1">{groupDetails.name}</div>
                                                            <div className="flex items-center gap-2 mb-3">
                                                                <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-widest">
                                                                    <Activity size={10} />
                                                                    {groupDetails.status || 'ACTIVE'}
                                                                </div>
                                                                <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-widest ${groupDetails.isPrivate ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-blue-500/10 border-blue-500/20 text-blue-400'}`}>
                                                                    {groupDetails.isPrivate ? <Shield size={10} /> : <Globe size={10} />}
                                                                    {groupDetails.isPrivate ? 'PRIVATE' : 'PUBLIC'}
                                                                </div>
                                                            </div>
                                                            <p className="text-zinc-400 text-sm leading-relaxed">{groupDetails.description || 'No description provided.'}</p>

                                                            {/* Group Stats */}
                                                            {stats && (
                                                                <div className="mt-4 flex items-center gap-2 bg-black/20 px-3 py-2 rounded-lg border border-white/5 w-fit">
                                                                    ...
                                                                    <div className="flex items-center gap-1.5">
                                                                        ...
                                                                        <span className="text-orange-400 font-medium text-xs">{stats.count} Queue</span>
                                                                    </div>
                                                                    <span className="text-white/10 text-xs">|</span>
                    ...
                    <AnimatePresence>
                        {showAddMember && (
                            <motion.div
                                ...
                                className="absolute inset-0 bg-[#0b141a] z-50 flex flex-col overflow-hidden rounded-2xl no-scrollbar"
                            >
                                <div className="h-16 px-6 bg-[#202c33] border-b border-white/5 flex items-center justify-between shrink-0">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={() => setShowAddMember(false)} className="text-[#aebac1] hover:text-white">
                                        <X size={24} />
                                    </button>
                                </div>

                                <div className="px-4 py-4 border-b border-white/5 bg-[#111b21] flex flex-col gap-4">
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-2 no-scrollbar">
                                    {filteredUsersToAdd.map(user => (
                                        <div key={user.id} className="flex items-center justify-between p-3 bg-[#111b21] rounded-xl hover:bg-[#202c33] transition-colors cursor-pointer group" onClick={() => handleAddMember(user.id)}>
                                            <div className="flex items-center gap-3">
                                            ...
                                            <button className="text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Plus size={20} />
                                            </button>
                                        </div>
                                    ))}

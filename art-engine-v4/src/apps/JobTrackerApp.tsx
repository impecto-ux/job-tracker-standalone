        ...
        <div className={`h-full w-full bg-black text-[#e9edef] font-sans flex flex-col overflow-hidden pb-12 md:flex-row ${isResizing ? 'cursor-col-resize select-none' : ''} relative`}>
            ...
            <GroupDiscoveryModal
                ...
                onJoined={(channelId: number | undefined) => {
                    refreshChannels(); // Force sidebar sync (stable)
                    if (channelId) {
                        setActiveChannel(channelId); // Stable
                    }
                }}
            />
            ...
            <div className={`flex-1 flex flex-col min-w-0 bg-zinc-950 overflow-hidden relative ${mobileTab === 'chat' ? 'hidden md:flex' : 'flex'}`}>
                            ...
                            {(!isMobile || mobileTab === 'tasks') && (
                                <div className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-zinc-900/50 shrink-0 backdrop-blur-sm z-[100]">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex bg-zinc-900 p-1 rounded-lg border border-white/5 gap-1 ml-2">
                                            <button
                                    ...
                                    <div className="w-full flex flex-col gap-6 pb-20 relative">
                                        ...
                                        <div className="space-y-2">
                                            {(() => {
                                                ...
                                                return sortedList.map((task, index) => {
                                                    const groupColorHsl = generateGroupColor(task.dept || 'General', 'bg').match(/hsl\([^)]+\)/)?.[0] || '#52525b';
                                                    const isSelected = selectedTaskIds.has(task.id);
                                                    const isMyRequest = auth.user && task.requester === auth.user.fullName;
                                                    ...
                                                    const handleRowClick = (e: React.MouseEvent) => {
                                                        ...
                                                        if (e.shiftKey && lastSelectedId !== null) {
                                                            const lastIndex = sortedList.findIndex(t => t.id === lastSelectedId);
                                                            if (lastIndex !== -1) {
                                                                const start = Math.min(lastIndex, index);
                                                                const end = Math.max(lastIndex, index);
                                                                const rangeIds = sortedList.slice(start, end + 1).map(t => t.id);
                                                        ...
                                                        // 3. Normal Click (Open Details)
                                                        setSelectedTask(task);
                                                        // Optional: Clear selection on normal click? 
                                                        // Windows behavior: Click clears selection and selects item.
                                                        // Here we stick to "Click opens detail" to not break existing flow.
                                                        // But if users want to Select simple item without opening? usage: Click checkbox.
                                                    };
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            {/* Desktop ID - Shifted right for checkbox */}
                                                            ...
                                                            <div className="hidden md:block col-span-2">
                                                                <div className="flex flex-wrap gap-1">
                                                                    ...
                                                                    {task.score > 0 && <span className="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider text-cyan-400 border border-cyan-500/30 bg-cyan-950/30">⚡ {task.score}</span>}
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="col-span-12 md:col-span-2 flex items-center gap-2 mt-1 md:mt-0">
                                                                ...
                                                                <span className={`text-[10px] font-black truncate uppercase tracking-tight ${task.status === 'done' ? 'text-emerald-400' : (task.owner === 'Unknown' ? 'text-zinc-600' : 'text-zinc-300')}`}>
                                                                    {task.status === 'done' ? `Done by ${task.owner}` : (task.owner === 'Unknown' ? 'Pool' : task.owner)}
                                                                </span>
                                                            </div>
                                                                        ...
                                                                        <div className="h-36 w-full shrink-0 relative flex items-center justify-center border-b border-white/5 bg-zinc-900/50"
                                                                            ...
                                                                            <div className="absolute bottom-2 left-3">
                                                                                <div className="flex items-center gap-2 text-xs text-zinc-500">
                                                                                    <span>Requested by {isMyRequest ? <span className="text-blue-200 font-bold shadow-blue-500/20 drop-shadow-sm">You</span> : task.requester}</span>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        {/* Footer Content */}
